...
/** Stable M05X1 framing for the exact persisted M05 counterexample document. */
final class M05CounterexampleCanonicalizer {
  CanonicalCounterexamples canonicalize(JsonNode persisted) {
    Objects.requireNonNull(persisted, "persisted");
    JsonNode scenarios = array(persisted, "scenarios");
    StringBuilder result = new StringBuilder();
    result
        .append("M05X1|schemaVersion=")
    ...
    for (int scenarioIndex = 0; scenarioIndex < scenarios.size(); scenarioIndex++) {
      ...
      for (int commandIndex = 0; commandIndex < original.size(); commandIndex++) {
        appendJson(result, "M05XO1", scenarioIndex, commandIndex, original.get(commandIndex));
        originalCommands++;
      }
      JsonNode commands = array(scenario, "commands");
      for (int commandIndex = 0; commandIndex < commands.size(); commandIndex++) {
        appendJson(result, "M05XC1", scenarioIndex, commandIndex, commands.get(commandIndex));
        minimizedCommands++;
      }
      appendJson(result, "M05XA1", scenarioIndex, -1, scenario.path("actualAtFailure"));
    }
  ...
  private static String integer(JsonNode node, String field) {
    ...
    if (!value.isIntegralNumber()) {
      throw malformed(field + " must be integer");
    }
  ...
  private static FixtureSchemaException malformed(String message) {
    return new FixtureSchemaException("malformed M05 counterexamples: " + message);
  }
